    ...
    <xsl:template match="/wadl:application">   
        <html>
            <head>
                ...
                <style type="text/css">
                    body {
						background: url('http://www.360i.com/images/homepage/logo_360i.png') no-repeat 15px 15px;
                        font-family: sans-serif;
                        font-size: 0.85em;
						margin: 0;
                        padding: 38px 190px;
						border-top: solid 7px #FFA535;
    ...
    <xsl:template match="wadl:resource" mode="list">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="name">
					<xsl:choose>
                        <xsl:when test="$context='/'"><xsl:value-of select="$context"/><xsl:value-of select="@path"/></xsl:when>
						<xsl:otherwise><xsl:value-of select="$context"/>/<xsl:value-of select="@path"/></xsl:otherwise>
					</xsl:choose>
